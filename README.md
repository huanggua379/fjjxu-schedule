...
> **登录账号**：使用学校官网「统一身份认证」的账号密码（即「今日校园」/教务系统同一套学号+密码），本 App 不单独注册账号。
>
> **首次使用必读**：正方教务系统没有校历接口，周次由「开学第一周周一」推算。**装好后请先到「设置 → 开学第一周（周一）」手动选择本学期准确的开学周一日期**，否则周次、今日课程与上课提醒的日期会偏差，课表也可能显示为空。
>
> 本项目由福建江夏学院在校生使用 Qwen-3.5-Max 制作，仅供学习交流，非学校官方应用。
...
| 模块 | 功能 | 说明 |
|------|------|------|
| 登录 | 学号 + 密码登录教务系统 | 学校统一身份认证账号（同「今日校园」）；金智 SSO，AES/CBC 加密口令，动态令牌；凭证仅存安全存储 |
...
2. **开学日期必须手动校准**：正方无官方校历接口，周次由「开学第一周周一」推算，默认只是按规则估算。**装好后务必到「设置 → 开学第一周（周一）」选择本学期准确的开学周一日期**，否则当前周判断错误会导致课表显示为空、今日课程与上课提醒日期整体偏差。
